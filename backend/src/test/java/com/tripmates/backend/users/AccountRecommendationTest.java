...
public class AccountRecommendationTest {
    ...
    public void testAccountWithFollowsHasUserAccountsRecommendations() throws Exception {
        ...
        mockMvc.perform(post("/users/" + lewisHamilton.getId() + "/follow")
                        .with(csrf())
                        .with(user(antonioFuoco.getEmail()).roles("USER")))
                .andExpect(status().isNoContent());

        String body =  mockMvc.perform(get("/users/recommendations/user/" + antonioFuoco.getId()).contentType(MediaType.APPLICATION_JSON))
        ...
        List<AccountResumeResponseDTO> accountResumeResponseDTOList = objectMapper.readValue(body, new TypeReference<List<AccountResumeResponseDTO>>() {});

        Assertions.assertEquals(1, accountResumeResponseDTOList.size());
        Assertions.assertEquals(List.of(AccountResumeResponseDTO.fromAccount(lewisHamilton)), accountResumeResponseDTOList);
    }
    ...
    public void testAccountThatFollowsAndUnfollowsAUserAccountsHasNoRecommendations() throws Exception {
        ...
        mockMvc.perform(post("/users/" + michaelJordan.getId() + "/follow")
                        .with(csrf())
                        .with(user(charlesLeclerc.getEmail()).roles("USER")))
                .andExpect(status().isNoContent());

        mockMvc.perform(post("/users/" + michaelJordan.getId() + "/unfollow")
                        .with(csrf())
                        .with(user(charlesLeclerc.getEmail()).roles("USER")))
                .andExpect(status().isNoContent());

        mockMvc.perform(get("/users/recommendations/user/" + charlesLeclerc.getId()).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());
    }
    ...
    public void testAccountWithCommonReviewsHasRecommendations() throws Exception {
        ...
        Review franReview = new Review(
                publication.getId(),
                "Excelente lugar",
                "Muy buen lugar, salen lindos amarillos. Lastima las habitaciones",
                4.5,
                List.of(),
                fran.getId()
        );

        Review pabloReview = new Review(
                publication.getId(),
                "Excelente lugar",
                "Muy buen lugar, salen lindas taruchas y algún doradito",
                5.0,
                List.of(),
                pablo.getId()
        );
        ...
        accountNodeRepository.createReviewed(fran.getId(), publication.getId(), franReview.getReviewId(), franReview.getRating());
        accountNodeRepository.createReviewed(pablo.getId(), publication.getId(), pabloReview.getReviewId(), pabloReview.getRating());

        String body =  mockMvc.perform(get("/users/recommendations/user/" + fran.getId()).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse()
                .getContentAsString();
