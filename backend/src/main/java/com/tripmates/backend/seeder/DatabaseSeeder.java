...
public class DatabaseSeeder implements CommandLineRunner {
	...
	private void registerAllUsers() {
		...
		try {
			...
			// Leer el JSON
			UserCredentialsWrapper wrapper = objectMapper.readValue(resource.getInputStream(),
					UserCredentialsWrapper.class);

			// Registrar cada usuario
			for (UserCredentials user : wrapper.getUsers()) {
				registerUser(user.getName(), user.getEmail(), user.getPassword(), user.getRole(),
	...
	private void seedPublications() {
		...
		createPublication("contacto@cafedelcentro.com", "Café de Especialidad",
				...
				List.of("café", "especialidad", "tostado"),
		...
		createPublication("contacto@cafedelcentro.com", "Tardes de Té",
				...
				List.of("té", "infusiones", "pastelería"),
				List.of("sample_images/publications/restaurant/cafe/cafe2.jpg"));

		// Hostel Montaña Mágica
		createPublication("info@hostelmontana.com", "Aventura en la Montaña",
		...
		createPublication("isabel@brisamarina.com", "Menú Degustación de Mariscos",
				...
				List.of("mariscos", "gourmet", "vista al mar"),
		...
		createPublication("isabel@brisamarina.com", "Cena con Vista al Atardecer",
				...
				List.of("romántico", "vista al mar", "cena gourmet"),
				List.of("sample_images/publications/restaurant/atardecer1.jpeg"));

		// Sabores Peruanos
		createPublication("gaston@saboresperuanos.com", "Especialidad: Ceviche Tradicional",
				...
				List.of(DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY, DayOfWeek.FRIDAY,
						DayOfWeek.SATURDAY),
				new AttentionSchedule(LocalTime.parse("12:00"), LocalTime.parse("23:30")), List.of(),
				List.of("ceviche", "comida peruana", "especialidad"),
				List.of("sample_images/publications/restaurant/ceviche1.jpg"));

		createPublication("gaston@saboresperuanos.com", "Noche de Pisco Sour",
				"Disfrutá de una noche de cócteles peruanos con música en vivo y tapas andinas.", "+54 11 4567-8901",
				"contacto@saboresperuanos.com", new Location("Av. Cabildo 2345, CABA", -34.5607, -58.4566),
				List.of(DayOfWeek.FRIDAY, DayOfWeek.SATURDAY),
				new AttentionSchedule(LocalTime.parse("20:00"), LocalTime.parse("01:00")), List.of(),
				List.of("pisco", "cócteles", "música en vivo"),
				List.of("sample_images/publications/restaurant/pisco1.jpg"));

		// El Encuentro Hostel
		createPublication("diego@elencuentrohostel.com", "Paquete Aventurero",
				...
				List.of(DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY, DayOfWeek.FRIDAY,
						DayOfWeek.SATURDAY, DayOfWeek.SUNDAY),
				new AttentionSchedule(LocalTime.parse("00:00"), LocalTime.parse("23:59")), List.of(),
				List.of("aventura", "montañismo", "excursiones"),
	...
	private void seedRoomPacks() {
		...
		addRoomPack("reservas@playadorada.com", new RoomPack(LocalDate.parse("2025-12-20"),
				...
				List.of()), List.of("sample_images/publications/hotel/deluxe1.jpeg"));

		// Hostel Montaña - 2 packs
		addRoomPack("info@hostelmontana.com", new RoomPack(LocalDate.parse("2025-12-20"), LocalDate.parse("2025-12-27"),
				2, List.of("wifi", "cocina_compartida", "area_comun"), 8000.0f,
				...
				List.of()), List.of("sample_images/business_picture/hostel1.jpg"));

		addRoomPack("info@hostelmontana.com", new RoomPack(LocalDate.parse("2025-12-20"), LocalDate.parse("2025-12-27"),
				2, List.of("wifi", "desayuno_simple", "bano_privado"), 15000.0f,
				"Habitación Doble Privada - Habitación privada con cama matrimonial y baño privado. Perfecta para parejas.",
				List.of()), List.of("sample_images/business_picture/hostel2.jpg"));
	}
	...
	private void seedMenuItems() {
		...
		// Café del Centro
		addMenuItem("contacto@cafedelcentro.com", "Café Especial", 1200.0f,
				"Café artesanal de granos tostados localmente. Servido con medialuna de manteca.",
				"sample_images/menu_items/cafe1.jpg");

		addMenuItem("contacto@cafedelcentro.com", "Té de Hierbas", 1000.0f,
				"Mezcla de hierbas aromáticas seleccionadas. Relajante y digestivo.",
				"sample_images/menu_items/cafe2.jpg");
	}

	private void addMenuItem(String businessEmail, String foodName, float price, String description, String imagePath) {
		try {
			...
			System.err
				.println("[MENU] Error creando ítem '" + foodName + "' para " + businessEmail + ": " + e.getMessage());
		}
	}
	...
	private void createPublication(String businessEmail, String title, String description, String phoneNumber,
		...
		try {
			...
			System.out.println("[PUBLICATION] Creada publicación '" + title + "' para " + businessEmail + " (id="
					+ created.id() + ")");
		}
		catch (Exception e) {
	...
	private void updateElEncuentroHostel() {
		...
		AccountUpdateRequestDTO dto = new AccountUpdateRequestDTO("El Encuentro Hostel",
				"Espacio para viajeros jóvenes que buscan conectar con otros aventureros", location, "+54 294 415-6789",
				"info@elencuentrohostel.com", AveragePrice.$, null, null, null, null, HotelType.Hostel, null);
	...
	private void updateBusinessImages() {
		...
		updateBusinessImagesForEmail("contacto@cafedelcentro.com", "sample_images/profile_pictures/cafe.jpeg",
				List.of("sample_images/business_picture/cafe1.jpg", "sample_images/business_picture/cafe2.jpg"));
		...
		updateBusinessImagesForEmail("isabel@brisamarina.com", "sample_images/profile_pictures/seafood.jpg",
				List.of("sample_images/business_picture/brisa1.jpeg", "sample_images/business_picture/brisa2.jpg"));
		...
		for (int i = 0; i < maxPubs; i++) {
			...
			for (int j = 0; j < reviewers.length; j++) {
				...
				try {
					...
					System.err.println(
							"[REVIEW] Error creando review de " + email + " para pub " + pubId + ": " + e.getMessage());
				}
			}
		}
	}
