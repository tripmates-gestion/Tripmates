...
public record PlanWithPublicationsResponseDTO(
    @Schema(description = "Plan's ID") String id,
...
) 
{
  public static PlanWithPublicationsResponseDTO fromPlan(Plan plan, PublicationRepository publicationRepository) {
    ...
    return new PlanWithPublicationsResponseDTO(
      plan.getId(), 
      plan.getName(), 
      plan.getDescription(), 
      plan.getOwnerId(), 
      new ArrayList<>(plan.getCollaboratorsUsersIds()),
      publications
    );
  }

  public static PlanWithPublicationsResponseDTO fromPlanMetadataWithContent(PlanMetadataWithContent planMetadataWithContent, PublicationRepository publicationRepository) {
    List<PublicationResumeResponseDTO> publications = new ArrayList<>();
    ...
    return new PlanWithPublicationsResponseDTO(
      planMetadataWithContent.planId(), 
      planMetadataWithContent.name(), 
      planMetadataWithContent.description(), 
      planMetadataWithContent.ownerId(), 
      new ArrayList<>(planMetadataWithContent.collaboratorsIds()),
      publications
    );
  }
}
