...
public record PublicationUpdateRequestDTO(
        @Schema(description = "Publication title (optional for update)") String title,
        ...
        @Schema(description = "Business exceptional closing days") @JsonSetter(nulls = Nulls.AS_EMPTY) List<LocalDate> exceptionalClosingDays,
        @Schema(description = "Business publication tags") @JsonSetter(nulls = Nulls.AS_EMPTY) List<String> tags,
        @Schema(description = "Optional list of 0-based photo indexes to delete from the publication") @JsonSetter(nulls = Nulls.AS_EMPTY) List<Integer> deletePhotoIndexes
) {
}
