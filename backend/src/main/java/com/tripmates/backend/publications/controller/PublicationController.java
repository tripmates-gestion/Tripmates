...
public class PublicationController {
    ...
    public ResponseEntity<?> uploadBusinessPublication(
            @Parameter(description = "JSON with the publication data. It should include: title, description, phone number, email, location, opening days, attention schedule, and exceptional closing days.")
            @RequestPart("data") String data,
            @Parameter(description = "Optional image files for the publication. Supported formats: JPG, PNG, etc.")
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Publication obtained successfully",
            content = @io.swagger.v3.oas.annotations.media.Content(
                mediaType = "application/json",
                schema = @io.swagger.v3.oas.annotations.media.Schema(implementation = com.tripmates.backend.publications.dto.BusinessPublicationResponseDTO.class)
            )
