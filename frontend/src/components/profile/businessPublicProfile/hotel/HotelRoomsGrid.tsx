...
import {
  ...
  export function HotelRoomsTab({ roomPacks, onEdit, onDelete }: Props) {
      ...
      return (
        <Box sx={{ textAlign: "center", py: 6, color: "text.secondary" }}>
          ...
          <Typography variant="body2">
            Actualmente no se encontraron opciones de alojamiento para las fechas
            seleccionadas.
          </Typography>
        </Box>
      );
                ...
                <Stack spacing={0.6} mb={1.5}>
                  <Stack direction="row" spacing={1.2} alignItems="center">
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {formatDate(pack.checkInDate)} —{" "}
                      {formatDate(pack.checkOutDate)}
                    </Typography>
                  </Stack>
                  <Stack direction="row" spacing={1.2} alignItems="center">
                    <People fontSize="small" color="action" />
                    <Typography variant="body2" color="text.secondary">
                      {pack.numberOfGuests} huésped
                      {pack.numberOfGuests > 1 ? "es" : ""}
                    </Typography>
                  </Stack>
                </Stack>
